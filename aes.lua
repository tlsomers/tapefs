...
--
function encrypt(password, data, keyLength, mode, iv, noPadding)
    ...
    if noPadding then
        ...
        paddedData = util.padByteString(data)
    end
...
-- mode and keyLength must be the same for encryption and decryption.
--
function decrypt(password, data, keyLength, mode, iv, noPadding)
	local mode = mode or CBCMODE
	local keyLength = keyLength or AES128
